...
class GameStory {
  ...
  List<String> getPeopleInRoom(String roomName){ // Need room name
  ...
  Set<String> getDialogues(String personName){
      //List<String> personDialogues = [];
    personName.replaceAll('\r', '');
    Set<String> personDialogues = <String>{''};
      for (var dialogue in dialogues) {
        print('***  '+dialogue.dialogue);
       // if (people.any((person) => person.name == personName && person.personiD == dialogue.personID && dialogue.sequenceOfDialogue ==0)) {
        if ( person.personiD == dialogue.personID && dialogue.sequenceOfDialogue ==0)) {

          print('&&&&&& ='+dialogue.dialogue);
          personDialogues.add(populateNameInDialogue(dialogue));
        }
      }
      return personDialogues;
    }
